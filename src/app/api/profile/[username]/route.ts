...
export async function POST(
  ...
  try {
    ...
    // Atomic update to prevent race conditions from parallel requests
    await User.updateOne(
      { _id: dbUser._id },
      { $pull: { forceRefreshTimestamps: { $lt: twentyFourHoursAgo } } }
    );

    const updatedUser = await User.findOneAndUpdate(
      {
        _id: dbUser._id,
        $expr: { $lt: [{ $size: "$forceRefreshTimestamps" }, 3] }
      },
      {
        $push: { forceRefreshTimestamps: now }
      },
      { new: true }
    );
    ...
    const recentRefreshes = (updatedUser.forceRefreshTimestamps || []).filter(
      (t: Date) => new Date(t).getTime() >= twentyFourHoursAgo.getTime()
    );
